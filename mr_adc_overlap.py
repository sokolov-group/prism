...
def compute_S12_0p_gno_projector(mr_adc, ignore_print = True):
    ...
    # Defining dimensions and S tensor
    S0p_dim = 1 + ncas * ncas * 2
    S12_dim = 1 + ncas * ncas
    S22_dim = ncas * ncas

    S_0p = np.zeros((S0p_dim, S0p_dim))
    ...
    Y[0,1:S12_dim] =- 0.5 * rdm_ca.reshape(-1).copy()
    Y[0,S12_dim:]  =- 0.5 * rdm_ca.reshape(-1).copy()
...
def compute_S12_p1p_gno_projector(mr_adc, ignore_print = True):
    ...
    S12_a_bab =- np.ascontiguousarray(S12_a_bba.transpose(0,1,3,2))
    S12_a_aaa =  np.ascontiguousarray(S12_a_bba + S12_a_bab)
    ...
    S22_aaa_bba = np.ascontiguousarray(S22_aaa_aaa - S22_bba_bba.transpose(0,2,1,3,5,4) + S22_bba_bba.transpose(0,1,2,3,5,4))
    S22_bba_aaa = np.ascontiguousarray(S22_aaa_bba.transpose(3,4,5,0,1,2))

    S22_aaa_bab =- np.ascontiguousarray(S22_aaa_bba.transpose(0,1,2,3,5,4))
    S22_bab_aaa =- np.ascontiguousarray(S22_bba_aaa.transpose(0,2,1,3,4,5))
    ...
    S_act[S_a_i:S_a_f, S_a_i:S_a_f] = S11_a_a

    S_act[S_a_i:S_a_f, S_aaa_i:S_aaa_f] = S12_a_aaa
    S_act[S_a_i:S_a_f, S_bba_i:S_bba_f] = S12_a_bba

    S_act[S_aaa_i:S_aaa_f, S_a_i:S_a_f] = S12_a_aaa.T
    S_act[S_bba_i:S_bba_f, S_a_i:S_a_f] = S12_a_bba.T

    S_act[S_aaa_i:S_aaa_f, S_aaa_i:S_aaa_f] = S22_aaa_aaa
    S_act[S_aaa_i:S_aaa_f, S_bba_i:S_bba_f] = S22_aaa_bba

    S_act[S_bba_i:S_bba_f, S_aaa_i:S_aaa_f] = S22_bba_aaa
    S_act[S_bba_i:S_bba_f, S_bba_i:S_bba_f] = S22_bba_bba
...
# Under development
def compute_S12_p1p_sanity_check_gno_projector_v2(mr_adc, ignore_print = True):
    ...
    # print(">>> SA (sanity-dev) S12 (a-aaa) norm: {:}".format(np.linalg.norm(S12[::2,::2,::2,::2])))
    # print(">>> SA (sanity-dev) S12 (a-bba) norm: {:}".format(np.linalg.norm(S12[::2,1::2,1::2,::2])))
    # print(">>> SA (sanity-dev) S12 (a-bab) norm: {:}".format(np.linalg.norm(S12[::2,1::2,::2,1::2])))

    # print(">>> SA (sanity-dev) S11 norm: {:}".format(np.linalg.norm(S11)))
    # print(">>> SA (sanity-dev) S12 norm: {:}".format(np.linalg.norm(S12)))
    # print(">>> SA (sanity-dev) S22 norm: {:}".format(np.linalg.norm(S22)))
    # print(">>> SA (sanity-dev) S_act sum: {:}".format(np.sum(S11) + 2*np.sum(S12) + np.sum(S22)))

    # np.save('SA_S22_v2', S22)
    S12 = S12[:,:,xy_ind[0],xy_ind[1]]
    S22 = S22[:,:,:,:,xy_ind[0],xy_ind[1]]
    S22 = S22[:,xy_ind[0],xy_ind[1]]
    ...
    # print(">>> SA (sanity-dev) S12 sum: {:}".format(np.sum(S12)))
    # print(">>> SA (sanity-dev) S_act norm: {:}".format(np.linalg.norm(S_act)))
    # print(">>> SA (sanity-dev) S_act sum: {:}".format(np.sum(np.abs(S_act))))
    ...
    np.save('SA_S_eval', S_eval)
    np.save('SA_S_evec', S_evec)
    ...
    S_p1p_12_inv_act = reduce(np.dot, (Y, S_evec, np.diag(S_inv_eval)))
    # S_p1p_12_inv_act = reduce(np.dot, (S_evec, np.diag(S_inv_eval)))
    np.save('S_p1p_12_inv_act_sa_dev', S_p1p_12_inv_act)
