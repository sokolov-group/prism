...
def compute_t2_0p_singles(mr_adc):
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        ## Two-electron integrals
        v_caec = np.ascontiguousarray(mr_adc.v2e.caec[:, :, :, s_chunk:f_chunk])
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        V1 += 1/2 * einsum('xyAa,Izaw,xyzw->IA', t1_aaee, v_caea, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp =- einsum('Ixay,zwAa,zywx->IA', t1_caea, v_aaee, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
        temp -= einsum('Ixay,zyAa,zx->IA', t1_caea, v_aaee, rdm_ca[s_chunk:f_chunk], optimize = einsum_type)
        temp += 1/2 * einsum('Ixya,zwAa,zywx->IA', t1_caae, v_aaee, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
        temp += 1/2 * einsum('Ixya,zyAa,zx->IA', t1_caae, v_aaee, rdm_ca[s_chunk:f_chunk], optimize = einsum_type)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp += 1/2 * einsum('Ixay,zAaw,zxwy->IA', t1_caea, v_aeea, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
        temp += 1/2 * einsum('Ixya,zAaw,zxyw->IA', t1_caae, v_aeea, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
        temp -= 1/2 * einsum('Ixay,zAaw,zw,xy->IA', t1_caea, v_aeea, rdm_ca[s_chunk:f_chunk], rdm_ca, optimize = einsum_type)
        temp += 1/4 * einsum('Ixya,zAaw,zw,xy->IA', t1_caae, v_aeea, rdm_ca[s_chunk:f_chunk], rdm_ca, optimize = einsum_type)
    ...
    for i_v_chunk, (s_v_chunk, f_v_chunk) in enumerate(chunks):
        ...
        for s_t_chunk, f_t_chunk in chunks:
            ...
            ## Amplitudes
            t1_caee = np.ascontiguousarray(mr_adc.t1.caee[:, s_t_chunk:f_t_chunk])

            ## Reduced density matrices
            rdm_ca = np.ascontiguousarray(mr_adc.rdm.ca[s_t_chunk:f_t_chunk, s_v_chunk:f_v_chunk])
...
def compute_t2_m1p_singles(mr_adc):
    ...
    chunks = tools.calculate_double_chunks(mr_adc, ncore, [ncore, nextern, nextern],
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        ## Amplitudes
        t1_ccae = mr_adc.t1.ccae[s_chunk:f_chunk]

        temp  = einsum('ijxa,iAja,Xx->XA', t1_ccae, v_cece, rdm_ca, optimize = einsum_type)
        temp -= 1/2 * einsum('ijxa,iajA,Xx->XA', t1_ccae, v_cece, rdm_ca, optimize = einsum_type)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        ## Two-electron integrals
        v_cace = mr_adc.v2e.cace[s_chunk:f_chunk]
        ...
        temp -= 1/2 * einsum('ijaA,ixja,Xx->XA', t1_ccee, v_cace, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('A,ijAa,ijxa,Xx->XA', e_extern, t1_ccee, t1_ccae, rdm_ca, optimize = einsum_type)
        temp -= 1/4 * einsum('A,ijaA,ijxa,Xx->XA', e_extern, t1_ccee, t1_ccae, rdm_ca, optimize = einsum_type)
        temp += einsum('a,ijxa,ijAa,Xx->XA', e_extern, t1_ccae, t1_ccee, rdm_ca, optimize = einsum_type)
        temp -= 1/2 * einsum('a,ijxa,ijaA,Xx->XA', e_extern, t1_ccae, t1_ccee, rdm_ca, optimize = einsum_type)
        temp -= einsum('i,ijxa,ijAa,Xx->XA', e_core_i, t1_ccae, t1_ccee, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('i,ijxa,ijaA,Xx->XA', e_core_i, t1_ccae, t1_ccee, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('j,ijxa,ijaA,Xx->XA', e_core, t1_ccae, t1_ccee, rdm_ca, optimize = einsum_type)
        temp -= einsum('j,ijxa,ijAa,Xx->XA', e_core, t1_ccae, t1_ccee, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('xy,ijxa,ijAa,Xy->XA', h_aa, t1_ccae, t1_ccee, rdm_ca, optimize = einsum_type)
        temp -= 1/4 * einsum('xy,ijxa,ijaA,Xy->XA', h_aa, t1_ccae, t1_ccee, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('xyzw,ijxa,ijAa,Xzyw->XA', v_aaaa, t1_ccae, t1_ccee, rdm_ccaa, optimize = einsum_type)
        temp -= 1/4 * einsum('xyzw,ijxa,ijaA,Xzyw->XA', v_aaaa, t1_ccae, t1_ccee, rdm_ccaa, optimize = einsum_type)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        ## Amplitudes
        t1_ce = mr_adc.t1.ce[s_chunk:f_chunk]
        t1_caea = mr_adc.t1.caea[s_chunk:f_chunk]
        t1_caae = mr_adc.t1.caae[s_chunk:f_chunk]
        ...
        V1 += temp
        mr_adc.log.timer_debug("contracting v2e.caee", *cput1)
    del(v_caee, t1_ce, t1_caea, t1_caae)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        V1 += temp
        mr_adc.log.timer_debug("contracting v2e.ceae", *cput1)
    del(v_ceae, t1_ce, t1_caea, t1_caae)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        V1 += temp
        mr_adc.log.timer_debug("contracting v2e.ceea", *cput1)
    del(v_ceea, t1_ce, t1_caea, t1_caae)

    chunks = tools.calculate_double_chunks(mr_adc, ncore, [ncas, nextern, nextern],
                                                                    [ncas, ncas, nextern], ntensors = 5)
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        cput1 = (logger.process_clock(), logger.perf_counter())
        mr_adc.log.debug("t1.caee [%i/%i], chunk [%i:%i]", i_chunk + 1, len(chunks), s_chunk, f_chunk)

        ## Molecular Orbitals Energies
        e_core = mr_adc.mo_energy.c[s_chunk:f_chunk]

        ## One-electron integrals
        h_ce = mr_adc.h1eff.ce[s_chunk:f_chunk]

        ## Two-electron integrals
        v_ceaa = mr_adc.v2e.ceaa[s_chunk:f_chunk]
        v_caae = mr_adc.v2e.caae[s_chunk:f_chunk]
        ...
        temp += 1/4 * einsum('A,ixAa,ia,Xx->XA', e_extern, t1_caee, t1_ce, rdm_ca, optimize = einsum_type)
        temp += 1/4 * einsum('A,ixAa,iyaz,Xyxz->XA', e_extern, t1_caee, t1_caea, rdm_ccaa, optimize = einsum_type)
        temp += 1/4 * einsum('A,ixAa,iyza,Xyzx->XA', e_extern, t1_caee, t1_caae, rdm_ccaa, optimize = einsum_type)
        ...
        temp -= 1/2 * einsum('A,ixaA,ia,Xx->XA', e_extern, t1_caee, t1_ce, rdm_ca, optimize = einsum_type)
        temp -= 1/2 * einsum('A,ixaA,iyaz,Xyxz->XA', e_extern, t1_caee, t1_caea, rdm_ccaa, optimize = einsum_type)
        temp += 1/4 * einsum('A,ixaA,iyza,Xyxz->XA', e_extern, t1_caee, t1_caae, rdm_ccaa, optimize = einsum_type)
        ...
        V1 += temp
        mr_adc.log.timer_debug("contracting t1.caee", *cput1)
    del(t1_caee, v_ceaa, v_caae, t1_caea, t1_caae)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp =- 1/2 * einsum('xa,yzAa,yXzx->XA', t1_ae, v_aaee, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
        temp += 1/2 * einsum('xa,yzAa,yz,Xx->XA', t1_ae, v_aaee, rdm_ca[s_chunk:f_chunk], rdm_ca, optimize = einsum_type)
        temp -= 1/2 * einsum('xyza,wzAa,wXxy->XA', t1_aaae, v_aaee, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
        temp += 1/2 * einsum('xyza,wuAa,wu,Xzyx->XA', t1_aaae, v_aaee, rdm_ca[s_chunk:f_chunk], rdm_ccaa, optimize = einsum_type)
        temp -= 1/2 * einsum('xyza,wuAa,wXzuyx->XA', t1_aaae, v_aaee, rdm_cccaaa[s_chunk:f_chunk], optimize = einsum_type)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp =- 1/2 * einsum('xa,zayA,zyxX->XA', t1_ae, v_aeae, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
        temp -= 1/2 * einsum('xyza,wAua,wuzXyx->XA', t1_aaae, v_aeae, rdm_cccaaa[s_chunk:f_chunk], optimize = einsum_type)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp =- 1/2 * einsum('xa,yAaz,yxXz->XA', t1_ae, v_aeea, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
        temp -= 1/2 * einsum('xyza,wAau,wxyXzu->XA', t1_aaae, v_aeea, rdm_cccaaa[s_chunk:f_chunk], optimize = einsum_type)
        temp -= 1/2 * einsum('xyza,waAz,wXyx->XA', t1_aaae, v_aeea, rdm_ccaa[s_chunk:f_chunk], optimize = einsum_type)
        temp -= 1/4 * einsum('xa,yAaz,yz,Xx->XA', t1_ae, v_aeea, rdm_ca[s_chunk:f_chunk], rdm_ca, optimize = einsum_type)
        temp -= 1/4 * einsum('xyza,wAau,wu,Xzyx->XA', t1_aaae, v_aeea, rdm_ca[s_chunk:f_chunk], rdm_ccaa, optimize = einsum_type)
    ...
    for i_v_chunk, (s_v_chunk, f_v_chunk) in enumerate(chunks):
        ...
        for s_t_chunk, f_t_chunk in chunks:
            ...
            ## Reduced density matrices
            rdm_ccaa = np.ascontiguousarray(mr_adc.rdm.ccaa[:, s_v_chunk:f_v_chunk, s_t_chunk:f_t_chunk])
...
def compute_t2_p1p_singles(mr_adc):
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp  = einsum('iXab,ibIa->IX', t1_caee, v_cece, optimize = einsum_type)
        temp -= 1/2 * einsum('ixab,ibIa,Xx->IX', t1_caee, v_cece, rdm_ca, optimize = einsum_type)
        temp -= 2 * einsum('iXab,iaIb->IX', t1_caee, v_cece, optimize = einsum_type)
        temp += einsum('ixab,iaIb,Xx->IX', t1_caee, v_cece, rdm_ca, optimize = einsum_type)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp  = einsum('iIba,iaXb->IX', t1_ccee, v_ceae, optimize = einsum_type)
        temp -= 1/2 * einsum('iIba,iaxb,Xx->IX', t1_ccee, v_ceae, rdm_ca, optimize = einsum_type)
        temp -= 2 * einsum('iIba,ibXa->IX', t1_ccee, v_ceae, optimize = einsum_type)
        temp += einsum('iIba,ibxa,Xx->IX', t1_ccee, v_ceae, rdm_ca, optimize = einsum_type)
...
def compute_t2_0pp_singles(mr_adc):
    ...
    def compute_V1__t1_m2(mr_adc, V1):
        ...
        V1_m2 -= 1/2 * einsum('a,xyab,zwab,XxyYzw->XY', e_extern, t1_aaee, t1_aaee, rdm_cccaaa, optimize = einsum_type)
        ...
        V1_m2 += 1/4 * einsum('xy,xzab,wuab,XwuYyz->XY', h_aa, t1_aaee, t1_aaee, rdm_cccaaa, optimize = einsum_type)
        V1_m2 += 1/4 * einsum('xy,xzab,wuab,XyzYwu->XY', h_aa, t1_aaee, t1_aaee, rdm_cccaaa, optimize = einsum_type)
        V1_m2 += 1/4 * einsum('Yxyz,xwab,uvab,Xywuzv->XY', v_aaaa, t1_aaee, t1_aaee, rdm_cccaaa, optimize = einsum_type)
        V1_m2 += 1/4 * einsum('Yxyz,xzab,wuab,Xywu->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccaa, optimize = einsum_type)
        V1_m2 += 1/4 * einsum('Yxyz,ywab,uvab,Xuvxzw->XY', v_aaaa, t1_aaee, t1_aaee, rdm_cccaaa, optimize = einsum_type)
        V1_m2 += 1/4 * einsum('Yxyz,zwab,uvab,Xywxuv->XY', v_aaaa, t1_aaee, t1_aaee, rdm_cccaaa, optimize = einsum_type)
        ...
        V1_m2 += 1/4 * einsum('xyzw,xuab,Yvba,Xzvuwy->XY', v_aaaa, t1_aaee, t1_aaee, rdm_cccaaa, optimize = einsum_type)
        V1_m2 -= 1/24 * einsum('xyzw,xuab,vsab,XywuYsvz->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/12 * einsum('xyzw,xuab,vsab,XywuYszv->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/24 * einsum('xyzw,xuab,vsab,XywuYvsz->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/6 * einsum('xyzw,xuab,vsab,XywuYvzs->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/12 * einsum('xyzw,xuab,vsab,XywuYzsv->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/12 * einsum('xyzw,xuab,vsab,XywuYzvs->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/24 * einsum('xyzw,xuab,vsab,XywusYzv->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XywusvYz->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/24 * einsum('xyzw,xuab,vsab,XywuvYzs->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XywuvsYz->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XywuzsYv->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XywuzsvY->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XywuzvYs->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XywuzvsY->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/24 * einsum('xyzw,xuab,vsab,XzvsYuwy->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/24 * einsum('xyzw,xuab,vsab,XzvsYuyw->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/12 * einsum('xyzw,xuab,vsab,XzvsYwuy->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/6 * einsum('xyzw,xuab,vsab,XzvsYwyu->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/12 * einsum('xyzw,xuab,vsab,XzvsYyuw->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/12 * einsum('xyzw,xuab,vsab,XzvsYywu->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XzvsuYwy->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XzvsuYyw->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XzvswYuy->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XzvswYyu->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XzvswuYy->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 += 1/24 * einsum('xyzw,xuab,vsab,XzvswuyY->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        V1_m2 -= 1/24 * einsum('xyzw,xuab,vsab,XzvsywYu->XY', v_aaaa, t1_aaee, t1_aaee, rdm_ccccaaaa, optimize = einsum_type)
        ...
        V1_m2 += 1/8 * einsum('xyzw,xzab,uvab,XuvYyw->XY', v_aaaa, t1_aaee, t1_aaee, rdm_cccaaa, optimize = einsum_type)
        V1_m2 += 1/8 * einsum('xyzw,xzab,uvab,XywYuv->XY', v_aaaa, t1_aaee, t1_aaee, rdm_cccaaa, optimize = einsum_type)
    ...
    def compute_V1__t1_m1(mr_adc, V1):
        ...
        for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
            cput1 = (logger.process_clock(), logger.perf_counter())
            mr_adc.log.debug("v2e.ceae t1.caee [%i/%i], chunk [%i:%i]", i_chunk + 1, len(chunks), s_chunk, f_chunk)
            ...
            V1_m1 -= 1/2 * einsum('a,iYab,ixab,Xx->XY', e_extern, t1_caee, t1_caee, rdm_ca, optimize = einsum_type)
            V1_m1 += 1/4 * einsum('a,iYab,ixba,Xx->XY', e_extern, t1_caee, t1_caee, rdm_ca, optimize = einsum_type)
            V1_m1 += 1/4 * einsum('a,iYba,ixab,Xx->XY', e_extern, t1_caee, t1_caee, rdm_ca, optimize = einsum_type)
            V1_m1 -= 1/2 * einsum('a,iYba,ixba,Xx->XY', e_extern, t1_caee, t1_caee, rdm_ca, optimize = einsum_type)
            V1_m1 -= 1/2 * einsum('a,ixab,iYab,Xx->XY', e_extern, t1_caee, t1_caee, rdm_ca, optimize = einsum_type)
            V1_m1 += 1/4 * einsum('a,ixab,iYba,Xx->XY', e_extern, t1_caee, t1_caee, rdm_ca, optimize = einsum_type)
            V1_m1 -= 1/2 * einsum('a,ixab,iyab,XxYy->XY', e_extern, t1_caee, t1_caee, rdm_ccaa, optimize = einsum_type)
            V1_m1 -= 1/2 * einsum('a,ixab,iyab,XyYx->XY', e_extern, t1_caee, t1_caee, rdm_ccaa, optimize = einsum_type)
            V1_m1 += 1/4 * einsum('a,ixab,iyba,XxYy->XY', e_extern, t1_caee, t1_caee, rdm_ccaa, optimize = einsum_type)
            V1_m1 += 1/4 * einsum('a,ixab,iyba,XyYx->XY', e_extern, t1_caee, t1_caee, rdm_ccaa, optimize = einsum_type)
            V1_m1 += 1/4 * einsum('a,ixba,iYab,Xx->XY', e_extern, t1_caee, t1_caee, rdm_ca, optimize = einsum_type)
            V1_m1 -= 1/2 * einsum('a,ixba,iYba,Xx->XY', e_extern, t1_caee, t1_caee, rdm_ca, optimize = einsum_type)
            V1_m1 += 1/4 * einsum('a,ixba,iyab,XxYy->XY', e_extern, t1_caee, t1_caee, rdm_ccaa, optimize = einsum_type)
            V1_m1 += 1/4 * einsum('a,ixba,iyab,XyYx->XY', e_extern, t1_caee, t1_caee, rdm_ccaa, optimize = einsum_type)
            V1_m1 -= 1/2 * einsum('a,ixba,iyba,XxYy->XY', e_extern, t1_caee, t1_caee, rdm_ccaa, optimize = einsum_type)
            V1_m1 -= 1/2 * einsum('a,ixba,iyba,XyYx->XY', e_extern, t1_caee, t1_caee, rdm_ccaa, optimize = einsum_type)
            ...
            V1_m1 += 1/2 * einsum('xyzw,ixab,iuab,XywYuz->XY', v_aaaa, t1_caee, t1_caee, rdm_cccaaa, optimize = einsum_type)
            V1_m1 += 1/2 * einsum('xyzw,ixab,iuab,XzuYwy->XY', v_aaaa, t1_caee, t1_caee, rdm_cccaaa, optimize = einsum_type)
            V1_m1 -= 1/4 * einsum('xyzw,ixab,iuba,XywYuz->XY', v_aaaa, t1_caee, t1_caee, rdm_cccaaa, optimize = einsum_type)
            V1_m1 -= 1/4 * einsum('xyzw,ixab,iuba,XzuYwy->XY', v_aaaa, t1_caee, t1_caee, rdm_cccaaa, optimize = einsum_type)

            mr_adc.log.timer_debug("contracting v2e.ceae t1.caee", *cput1)
        del(t1_caee, v_ceae)
    ...
    def compute_V1__t1_0(mr_adc, V1):
        ...
        for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
            cput1 = (logger.process_clock(), logger.perf_counter())
            mr_adc.log.debug("v2e.cece t1.ccee [%i/%i], chunk [%i:%i]", i_chunk + 1, len(chunks), s_chunk, f_chunk)
            ...
            V1_0 += einsum('ijab,ibja,XY->XY', t1_ccee, v_cece, rdm_ca, optimize = einsum_type)
            ...
            V1_0 += einsum('a,ijab,ijba,XY->XY', e_extern, t1_ccee, t1_ccee, rdm_ca, optimize = einsum_type)
            ...
            mr_adc.log.timer_debug("contracting v2e.cece t1.ccee", *cput1)
        del(v_cece, t1_ccee)
