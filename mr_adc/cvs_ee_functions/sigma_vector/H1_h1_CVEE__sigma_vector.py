...
def compute_sigma_vector__H1__h1_h1__CCEE_CVEE(mr_adc, X, sigma):
    ...
    chunks = tools.calculate_chunks(mr_adc, mr_adc.nval, [mr_adc.ncvs, mr_adc.nextern, mr_adc.nextern], ntensors = 2)
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp  = 2 * einsum('KiCa,iaDL->KLCD', X_i, v_veex, optimize = einsum_type)
        temp -= einsum('KiaC,iaDL->KLCD', X_i, v_veex, optimize = einsum_type)
        temp += 2 * einsum('LiDa,iaCK->KLCD', X_i, v_veex, optimize = einsum_type)
        temp -= einsum('LiaD,iaCK->KLCD', X_i, v_veex, optimize = einsum_type)

        sigma_KLCD += temp
        mr_adc.log.timer_debug("v2e.veex contractions", *cput2)
    del(v_veex, temp, X_i)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp =- einsum('KiCa,iLDa->KLCD', X_i, v_vxee, optimize = einsum_type)
        temp -= einsum('KiaD,iLCa->KLCD', X_i, v_vxee, optimize = einsum_type)
        temp -= einsum('LiDa,iKCa->KLCD', X_i, v_vxee, optimize = einsum_type)
        temp -= einsum('LiaC,iKDa->KLCD', X_i, v_vxee, optimize = einsum_type)

        sigma_KLCD += temp
        mr_adc.log.timer_debug("v2e.vxee contractions", *cput2)
    del(v_vxee, temp, X_i)
...
def compute_sigma_vector__H1__h1_h1__CAEE_CVEE(mr_adc, X, sigma):
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp = einsum('KiCa,iaDx,Wx->KWCD', X_i, v_veea, rdm_ca, optimize = einsum_type)
        temp -= 1/2 * einsum('KiaC,iaDx,Wx->KWCD', X_i, v_veea, rdm_ca, optimize = einsum_type)

        sigma_KWCD += temp
        mr_adc.log.timer_debug("v2e.veea contractions", *cput2)
    del(v_veea, temp, X_i)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp =- 1/2 * einsum('KiCa,ixDa,Wx->KWCD', X_i, v_vaee, rdm_ca, optimize = einsum_type)
        temp -= 1/2 * einsum('KiaD,ixCa,Wx->KWCD', X_i, v_vaee, rdm_ca, optimize = einsum_type)

        sigma_KWCD += temp
        mr_adc.log.timer_debug("v2e.vaee contractions", *cput2)
    del(v_vaee, temp, X_i)
...
def compute_sigma_vector__H1__h1_h1__CAAA_CVEE(mr_adc, X, sigma):
    ...
    sigma_KWUV_aaaa = zeros((mr_adc.ncvs, mr_adc.ncas, mr_adc.ncas, mr_adc.ncas))
    sigma_KWUV_abab = zeros((mr_adc.ncvs, mr_adc.ncas, mr_adc.ncas, mr_adc.ncas))
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        ## Two-electron integral
        v_veae = mr_adc.v2e.veae[s_chunk:f_chunk]

        X_i = ascontiguousarray(X[:, s_chunk:f_chunk])

        temp =- 1/2 * einsum('Kiab,iaUb,VW->KWUV', X_i, v_veae, rdm_ca, optimize = einsum_type)
        temp += einsum('Kiab,ibUa,VW->KWUV', X_i, v_veae, rdm_ca, optimize = einsum_type)
        sigma_KWUV_abab += temp

        temp += 1/2 * einsum('Kiab,iaVb,UW->KWUV', X_i, v_veae, rdm_ca, optimize = einsum_type)
        temp -= einsum('Kiab,ibVa,UW->KWUV', X_i, v_veae, rdm_ca, optimize = einsum_type)
        sigma_KWUV_aaaa += temp

        mr_adc.log.timer_debug("v2e.veae contractions", *cput2)
    del(v_veae, temp, X_i)
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp =- 1/2 * einsum('Kiab,a,iUab,VW->KWUV', X_i, e_extern, t1_vaee, rdm_ca, optimize = einsum_type)
        temp += einsum('Kiab,a,iUba,VW->KWUV', X_i, e_extern, t1_vaee, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('Kiab,a,iVab,UW->KWUV', X_i, e_extern, t1_vaee, rdm_ca, optimize = einsum_type)
        temp -= einsum('Kiab,a,iVba,UW->KWUV', X_i, e_extern, t1_vaee, rdm_ca, optimize = einsum_type)
        temp -= 1/2 * einsum('Kiab,b,iUab,VW->KWUV', X_i, e_extern, t1_vaee, rdm_ca, optimize = einsum_type)
        temp += einsum('Kiab,b,iUba,VW->KWUV', X_i, e_extern, t1_vaee, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('Kiab,b,iVab,UW->KWUV', X_i, e_extern, t1_vaee, rdm_ca, optimize = einsum_type)
        temp -= einsum('Kiab,b,iVba,UW->KWUV', X_i, e_extern, t1_vaee, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('Kiab,i,iUab,VW->KWUV', X_i, e_val, t1_vaee, rdm_ca, optimize = einsum_type)
        temp -= einsum('Kiab,i,iUba,VW->KWUV', X_i, e_val, t1_vaee, rdm_ca, optimize = einsum_type)
        temp -= 1/2 * einsum('Kiab,i,iVab,UW->KWUV', X_i, e_val, t1_vaee, rdm_ca, optimize = einsum_type)
        temp += einsum('Kiab,i,iVba,UW->KWUV', X_i, e_val, t1_vaee, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('Kiab,Ux,ixab,VW->KWUV', X_i, h_aa, t1_vaee, rdm_ca, optimize = einsum_type)
        temp -= einsum('Kiab,Ux,ixba,VW->KWUV', X_i, h_aa, t1_vaee, rdm_ca, optimize = einsum_type)
        temp -= 1/2 * einsum('Kiab,Vx,ixab,UW->KWUV', X_i, h_aa, t1_vaee, rdm_ca, optimize = einsum_type)
        temp += einsum('Kiab,Vx,ixba,UW->KWUV', X_i, h_aa, t1_vaee, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('Kiab,ixab,UxVy,Wy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('Kiab,ixab,Uxyz,VzWy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/2 * einsum('Kiab,ixab,UyVx,Wy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ca, optimize = einsum_type)
        temp -= 1/6 * einsum('Kiab,ixab,Uyzx,VyWz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 1/6 * einsum('Kiab,ixab,Uyzx,VyzW->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/2 * einsum('Kiab,ixab,Vxyz,UzWy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 1/6 * einsum('Kiab,ixab,Vyzx,UyWz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/6 * einsum('Kiab,ixab,Vyzx,UyzW->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/6 * einsum('Kiab,ixab,Wyxz,UVyz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 1/6 * einsum('Kiab,ixab,Wyxz,UVzy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= einsum('Kiab,ixba,UxVy,Wy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ca, optimize = einsum_type)
        temp -= einsum('Kiab,ixba,Uxyz,VzWy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += einsum('Kiab,ixba,UyVx,Wy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ca, optimize = einsum_type)
        temp += 1/3 * einsum('Kiab,ixba,Uyzx,VyWz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/3 * einsum('Kiab,ixba,Uyzx,VyzW->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += einsum('Kiab,ixba,Vxyz,UzWy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/3 * einsum('Kiab,ixba,Vyzx,UyWz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 1/3 * einsum('Kiab,ixba,Vyzx,UyzW->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 1/3 * einsum('Kiab,ixba,Wyxz,UVyz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/3 * einsum('Kiab,ixba,Wyxz,UVzy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        sigma_KWUV_aaaa += temp
        ...
        temp += 1/2 * einsum('Kiab,ixab,UxVy,Wy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ca, optimize = einsum_type)
        temp += 1/2 * einsum('Kiab,ixab,Uxyz,VzWy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/3 * einsum('Kiab,ixab,Uyzx,VyWz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/6 * einsum('Kiab,ixab,Uyzx,VyzW->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/6 * einsum('Kiab,ixab,Vyzx,UyWz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/3 * einsum('Kiab,ixab,Vyzx,UyzW->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 1/6 * einsum('Kiab,ixab,Wyxz,UVyz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 1/3 * einsum('Kiab,ixab,Wyxz,UVzy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= einsum('Kiab,ixba,UxVy,Wy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ca, optimize = einsum_type)
        temp -= einsum('Kiab,ixba,Uxyz,VzWy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 2/3 * einsum('Kiab,ixba,Uyzx,VyWz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 1/3 * einsum('Kiab,ixba,Uyzx,VyzW->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 1/3 * einsum('Kiab,ixba,Vyzx,UyWz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp += 2/3 * einsum('Kiab,ixba,Vyzx,UyzW->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 1/3 * einsum('Kiab,ixba,Wyxz,UVyz->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        temp -= 2/3 * einsum('Kiab,ixba,Wyxz,UVzy->KWUV', X_i, t1_vaee, v_aaaa, rdm_ccaa, optimize = einsum_type)
        sigma_KWUV_abab += temp

        mr_adc.log.timer_debug("t1.vaee contractions", *cput2)
    del(t1_vaee, temp, X_i)
...
def compute_sigma_vector__H1__h1_h1__CAEA_CVEE(mr_adc, X, sigma):
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp =- 1/2 * einsum('Kiab,iaCb,UW->KWCU', X_i, v_veee, rdm_ca, optimize = einsum_type)
        temp += einsum('Kiab,ibCa,UW->KWCU', X_i, v_veee, rdm_ca, optimize = einsum_type)
...
def compute_sigma_vector__H1__h1_h1__CVEA_CVEE(mr_adc, X, sigma):
    ...
    sigma_KLCW_abab = ascontiguousarray(sigma_KLCW_abab.transpose(3,0,1,2))
    sigma_KLCW_baab = ascontiguousarray(sigma_KLCW_baab.transpose(3,0,1,2))

    # v_aeee
    chunks = tools.calculate_chunks(mr_adc, mr_adc.ncas, [mr_adc.nextern, mr_adc.nextern, mr_adc.nextern])
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        cput2 = (logger.process_clock(), logger.perf_counter())
        ...
        sigma_KLCW_abab[s_chunk:f_chunk] += einsum('KLab,WbCa->WKLC', X, v_aeee, optimize = einsum_type)
        sigma_KLCW_abab -= 1/2 * einsum('KLab,xbCa,xW->WKLC', X, v_aeee, rdm_ca, optimize = einsum_type)

        sigma_KLCW_baab[s_chunk:f_chunk] -= einsum('KLab,WaCb->WKLC', X, v_aeee, optimize = einsum_type)
        sigma_KLCW_baab += 1/2 * einsum('KLab,xaCb,xW->WKLC', X, v_aeee, rdm_ca, optimize = einsum_type)

        mr_adc.log.timer_debug("computing v2e.aeee", *cput2)
    del(v_aeee, rdm_ca)

    sigma[cvea__abab] += ascontiguousarray(sigma_KLCW_abab.transpose(1,2,3,0)).reshape(-1)
    sigma[cvea__baab] += ascontiguousarray(sigma_KLCW_baab.transpose(1,2,3,0)).reshape(-1)
...
def compute_sigma_vector__H1__h1_h1__CVEE_CVEE(mr_adc, X, sigma):        
    ...
    for i_chunk, (s_chunk, f_chunk) in enumerate(chunks):
        ...
        temp  = 2 * einsum('KiCa,iaDL->KLCD', X_i, v_veev, optimize = einsum_type)
        temp -= einsum('KiaC,iaDL->KLCD', X_i, v_veev, optimize = einsum_type)
    ...
    sigma_KLCD += temp.reshape(nextern, nextern, ncvs, nval).transpose(2,3,0,1)
    del(X, temp)
