...
def multiseed_casscf(mc, mo_seed, deg_tol=1e-3, angles=(0, 30, 60, 90), log=None):
    ...
    if not deg_pairs:
        log.info("multiseed_casscf: no degenerate active pairs (min occ gap=%.2e); "
                 "single kernel call." % np.min(np.abs(np.diff(occ))))
        mc.kernel(mo_seed)
        return mc

    log.info("multiseed_casscf: %d degenerate pair(s) %s; running %d seeds."
    ...
    if abs(mc.e_tot - best_e) > 1e-10:
        mc.kernel(best_mo)
    log.info("multiseed_casscf -> best SA-energy = %.10f Ha" % best_e)
    return mc
...
def project_amo_manually(old_mo_coeff, ncas, ncore, new_fock, norb, log=None):
    ...
    log.info("project_amo: fidelity = %s" % fidelity)
