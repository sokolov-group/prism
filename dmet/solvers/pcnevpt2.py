...
def solve(const, oei, fock, tei, norb, nel, nimp, dm_guess_rhf,
    ...
    if cas_select not in ('energy', 'natorb'):
        raise ValueError(
            f"pcnevpt2::solve: unknown cas_select='{cas_select}'. Valid: ['energy', 'natorb']")
    ...
    with ctx:
        ...
        if embed_level_shift != 0.0:
            ...
            mf.level_shift = 0.0
            mf.scf(mf.make_rdm1())
            log.info("pcnevpt2::solve : level-shift verification: E(shift=%s)=%.10f  "
        ...
        if rohf_stability and _use_rohf:
            _stabilize_rohf(mf, tag='pcnevpt2::solve', log=log)
        if _use_rohf:
            log.info("pcnevpt2::solve : embedded ROHF (spin=%d, nel=%d, norb=%d)"
                     % (_spin, nel, norb))
        ...
        if mo_natorb is not None:
            ...
            log.info("pcnevpt2::solve : CAS selection by %s, CAS(%d,%d)"
        ...
        if sa_nstates > 1:
            log.info("\npcnevpt2::solve : embedded SA-CASSCF (%d states, ncas=%d, nelecas=%d)"
            ...
            log.info("\npcnevpt2::solve : embedded CASSCF energy = %.10f Ha" % mc.e_tot)
        ...
        log.info("\npcnevpt2::solve : PC-NEVPT2 state energies (excitation = state - state 0):")
