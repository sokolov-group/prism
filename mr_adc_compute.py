...
def kernel(mr_adc):
    ...
    apply_M, precond, x0 = setup_davidson(mr_adc, t_amp)
    ...
    spec_intensity = compute_trans_properties(mr_adc, t_amp, E, U)
...
def setup_davidson(mr_adc, t_amp):
    ...
    if mr_adc.method_type == "ip":
        ...
        M_00 = mr_adc_ip.compute_M_00(mr_adc, t_amp)

        # Compute parts of the h0-h1 block of the effective Hamiltonian matrix
        if mr_adc.method in ("mr-adc(2)", "mr-adc(2)-x"):
            M_01 = mr_adc_ip.compute_M_01(mr_adc, t_amp)

    elif mr_adc.method_type == "ea":
        # Compute h0-h0 block of the effective Hamiltonian matrix
        M_00 = mr_adc_ea.compute_M_00(mr_adc, t_amp)

        # Compute parts of the h0-h1 block of the effective Hamiltonian matrix
        if mr_adc.method in ("mr-adc(2)", "mr-adc(2)-x"):
            M_01 = mr_adc_ea.compute_M_01(mr_adc, t_amp)

    elif mr_adc.method_type == "ee":
        # Compute h0-h0 block of the effective Hamiltonian matrix
        M_00   = mr_adc_ee.compute_M_00(mr_adc, t_amp)

        # Compute parts of the h0-h1 and h1-h1 blocks of the effective Hamiltonian matrix
        if mr_adc.method in ("mr-adc(2)", "mr-adc(2)-x"):
            M_01 = mr_adc_ee.compute_M_01(mr_adc, t_amp)

    elif mr_adc.method_type == "cvs-ip":
        # Compute h0-h0 block of the effective Hamiltonian matrix
        M_00 = mr_adc_cvs_ip.compute_M_00(mr_adc, t_amp)

        # Compute parts of the h0-h1 block of the effective Hamiltonian matrix
        if mr_adc.method in ("mr-adc(2)", "mr-adc(2)-x"):
            M_01 = mr_adc_cvs_ip.compute_M_01(mr_adc, t_amp)

    elif mr_adc.method_type == "cvs-ee":
        # Compute h0-h0 block of the effective Hamiltonian matrix
        M_00 = mr_adc_cvs_ee.compute_M_00(mr_adc, t_amp)

    # Compute diagonal of the M matrix
    if mr_adc.method_type == "ip":
        precond = mr_adc_ip.compute_preconditioner(mr_adc, t_amp, M_00)
    elif mr_adc.method_type == "ea":
        precond = mr_adc_ea.compute_preconditioner(mr_adc, t_amp, M_00)
    elif mr_adc.method_type == "ee":
    ...
    apply_M = None
    if mr_adc.method_type == "ip":
        apply_M = mr_adc_ip.define_effective_hamiltonian(mr_adc, t_amp, M_00, M_01, M_11)
    elif mr_adc.method_type == "ea":
        apply_M = mr_adc_ea.define_effective_hamiltonian(mr_adc, t_amp, M_00, M_01, M_11)
    elif mr_adc.method_type == "ee":
        apply_M = mr_adc_ee.define_effective_hamiltonian(mr_adc, t_amp, M_00, M_01)
    elif mr_adc.method_type == "cvs-ip":
        apply_M = mr_adc_cvs_ip.define_effective_hamiltonian(mr_adc, t_amp, M_00, M_01)
    elif mr_adc.method_type == "cvs-ee":
        apply_M = mr_adc_cvs_ee.define_effective_hamiltonian(mr_adc, t_amp, M_00)
...
def compute_trans_properties(mr_adc, t_amp, E, U):
    ...
    if mr_adc.method_type == "ip":
        T = mr_adc_ip.compute_trans_moments(mr_adc, t_amp)
    elif mr_adc.method_type == "cvs-ip":
        T = mr_adc_cvs_ip.compute_trans_moments(mr_adc, t_amp)
    elif mr_adc.method_type == "ea":
        T = mr_adc_ea.compute_trans_moments(mr_adc, t_amp)
    elif mr_adc.method_type == "ee":
        T = mr_adc_ee.compute_trans_moments(mr_adc, t_amp)
    elif mr_adc.method_type == "cvs-ee":
        T = mr_adc_cvs_ee.compute_trans_moments(mr_adc, t_amp)
    else:
